    		...
    		if (gps_waiting (&gps_data, 20000000)) 
		{
			...
			else
			{
		        	...
		        	!isnan(gps_data.fix.longitude))
				{
					...
					//need modify
					m_speed = (double)gps_data.fix.speed;
					return GPS(gps_data.fix.latitude, gps_data.fix.longitude);
...
IR::build_beacon(struct BeaconMessage *message)
{
	...
	q_gps.push(m_gps);
	message->src_gps_x = GPS::GPSXToUint32(m_gps);
	message->src_gps_y = GPS::GPSYToUint32(m_gps);
	//m_speed(double)
	if(q_gps.size() == 2)
	//Speed speed = GPSVec::SpeedChange(q_gps,m_speed);
	message->src_speed_x = Speed::DoubleToUint32((double)GPSVec::SpeedChange(q_gps,m_speed).x);
	message->src_speed_y = Speed::DoubleToUint32((double)GPSVec::SpeedChange(q_gps,m_speed).y);
	gettimeofday(&tv, &tz);
	//message->curr_time = tv.tv_sec;
	printf("gps in beacon: %lf,%lf\n",m_gps.x,m_gps.y);
}
...
IR::init(int argc, char * argv[])
{
	...
	//data_recv_buffer = (char*)malloc(sizeof(char) * FRAME_SIZE);
	...
	m_dataSA.sin_addr.s_addr = htonl(INADDR_ANY);
	m_dataSA.sin_port = htons(DATA_PORT);
	bzero(&(m_dataSA.sin_zero),8);
	data_fd = socket(AF_INET, SOCK_DGRAM, 0);//IPPROTO_UDP
	setsockopt(data_fd, SOL_SOCKET, SO_REUSEADDR|SO_BROADCAST, &so_broadcast, sizeof(so_broadcast));
	if (data_fd < 0)
...
IR::schedule()
{
	...
	while (1)
	{
		...
		retn = select(maxfd, &rset, NULL, NULL, &tvp);
...
int
IR::gps_by_ip(uint32_t dst_vir_ip,in_addr * dst_phy_ip,GPS * dst_gps)
{
	...
	if (it != m_forward_table.end())
	{
		*dst_gps = it->second->m_gps;
		dst_phy_ip->s_addr = it->second->GetNextHopIP();
		struct in_addr tl;
		tl.s_addr = it->second->GetNextHopIP();
		printf("jixuefeng: %s",inet_ntoa(tl));
		return 1;
	}
	else
	{
...
IR::recv_tun(int fd)
{
	...
	message.src_gps_y = GPS::GPSYToUint32(m_gps);
	message.ttl = 16;
	printf("++++dst_phy_ip: %s",inet_ntoa(dst_phy_ip));
	if(opt)
	{
		struct sockaddr_in dst_addr;
		dst_addr.sin_family = AF_INET;
		dst_addr.sin_port = htons(DATA_PORT);
		dst_addr.sin_addr.s_addr = dst_phy_ip.s_addr;
		message.dst_gps_x = dst_gps.x;
		message.dst_gps_y = dst_gps.y;
		printf("send data unicast: myGPS (%d,%d), dstGPS (%d,%d)\n",message.src_gps_x,message.src_gps_y = m_gps.y,message.dst_gps_x,message.dst_gps_y);
		memcpy(data_buffer, tun_buffer, nread);
		memcpy(data_buffer+nread, (const char*)&message, sizeof(message));


		struct DataMessage *data_message = (struct DataMessage*)(data_buffer + nread);
		...
		std::cout << "dst_ip:" <<inet_ntoa(dst_addr_t) << std::endl;
		dst_addr_t.s_addr = src_ip;
		std::cout << "src_ip:"<< inet_ntoa(dst_addr_t) << std::endl;

		int nsend = sendto(data_fd, data_buffer, nread + sizeof(DataMessage), 0, (struct sockaddr*) &dst_addr, sizeof(dst_addr));//nread + sizeof(message)
		std::cout << "sendto bytes:" << nsend << std::endl;
		//sendto(data_fd, (char *)&message, sizeof(message), 0, (struct sockaddr*) &dst_addr, sizeof(dst_addr));
		//sendto(data_fd, tun_buffer, nread /*sizeof(tun_buffer)*/, 0, (struct sockaddr*) &dst_addr, sizeof(dst_addr));
	}
	else
	{
		broadcast_addr.sin_port = htons(DATA_PORT);
		message.dst_gps_x = 0;
		message.dst_gps_y = 90;
		printf("send data broadcast: myGPS (%d,%d), dstGPS (%d,%d)\n",message.src_gps_x,message.src_gps_y = m_gps.y,message.dst_gps_x,message.dst_gps_y);
		...
		dst_addr_t.s_addr = src_ip;
		std::cout << "src_ip:"<< inet_ntoa(dst_addr_t) << std::endl;
		int nsend = sendto(data_fd, data_buffer, nread + sizeof(message), 0, (struct sockaddr*) &broadcast_addr, sizeof(broadcast_addr));
		std::cout << "sendto bytes:" << nsend << std::endl;
		if ( nsend < 0)
		{
			perror("sendto data");
		...
		//sendto(data_fd, tun_buffer, nread /*sizeof(tun_buffer)*/, 0, (struct sockaddr*) &broadcast_addr, sizeof(broadcast_addr));
	}
    ...
    for(std::map<uint32_t, NeighborEntry*>::iterator iter = m_neighbor_table.begin();iter != m_neighbor_table.end();)
        {
            ...
            else
            {
                ...
                //GPSVec selfToDstVec(selfGPS, dstGPS);
				//GPSVec selfToNghbrVec(selfGPS, nghbrGPS);
				double angle = GPSVec::CalTheate(selfGPS,nghbrGPS,dstGPS);
				if (angle < CALNEXTHOPANGLE)
				{
					...
					{
						disMax = dis;
						nextHopID = iter->second->GetPhyIP();
					}
				}
                iter++;
            }
        ...
        if (angle < CALNEXTHOPANGLE)
			{
				...
				{
					disMax = dis;
					std::map<uint32_t, NeighborEntry*>::iterator it = m_neighbor_table.find(iter->first);
					nextHopID = it->second->GetPhyIP();
				}
			}
    }
    const double EPSINON = 0.00000001;
...
void
IR::Forwarding(uint32_t dstIP, GPS dstGPS, int nread)
{
    uint32_t nextHopID = 0;
    std::map<uint32_t, ForwardTableEntry*>::iterator it = m_forward_table.find(dstIP);
    struct timeval now;
    gettimeofday(&now,NULL);
    int until_time = (now.tv_sec - begin_time)*1000 + now.tv_usec/1000;
    if(it == m_forward_table.end())
    ...
    else if(until_time - (int)it->second->GetTimestamp() > TTLNEIGHBORTABLE)
    {
	 const double EPSINON = 0.00000001;
        ...
        if (nextHopID != BROADCASTADDRESS)
        {
            m_forward_table[dstIP]->m_next_hop_ip = nextHopID;
            m_forward_table[dstIP]->m_timestamp = until_time;
        }
    }
    else
    {
    ...
    {
        ///send "nextHopID --- IP"
	//std::map<uint32_t, NeighborEntry*>::iterator it = m_neighbor_table.find(dstIP);
	my_send(nextHopID, nread);
    }
}

void IR::data_broadcast(int nread)
{
	broadcast_addr.sin_port = htons(DATA_PORT);
	int nsend = sendto(data_fd, data_recv_buffer, nread + sizeof(DataMessage), 0, (struct sockaddr*) &broadcast_addr, sizeof(broadcast_addr));
}

void IR::my_send(uint32_t dst_phy_ip, int nread)
{
	struct sockaddr_in dst_addr;
	dst_addr.sin_family = AF_INET;
	dst_addr.sin_port = htons(DATA_PORT);
	dst_addr.sin_addr.s_addr = dst_phy_ip;
	int nsend = sendto(data_fd, data_recv_buffer, nread + sizeof(DataMessage), 0, (struct sockaddr*) &dst_addr, sizeof(dst_addr));//nread + sizeof(message)
}
...
IR::recv_data(int fd)
{
	...
	int count;
	int nread;
	nread = recvfrom(fd, data_recv_buffer, sizeof(data_recv_buffer), MSG_OOB, (struct sockaddr*) &from_addr, &from_len);
	data_message = (struct DataMessage*)(data_recv_buffer +nread - sizeof(DataMessage));
	//printf("size of data_message%ld\n",sizeof(DataMessage));
	struct iphdr *p_ip;
	p_ip = (struct iphdr *) data_recv_buffer;
	dst_ip = p_ip->daddr;
	src_ip = p_ip->saddr;
	//src_ip = from_addr.sin_addr.s_addr;
	struct in_addr addr_t;
	if(src_ip != m_vir_ip.s_addr)
	{
		printf("----------------------------------------------------------\n");
		printf("receive data\n");
		std::cout << "nread:" << nread << " from:" << inet_ntoa(from_addr.sin_addr) << std::endl;
		std::cout << data_message->length << "," << data_message->src_gps_x << "," << data_message->src_gps_y << "," << data_message->dst_gps_x << "," << data_message->dst_gps_y << std::endl;
		addr_t.s_addr = dst_ip;
		std::cout << "dst_ip:" <<inet_ntoa(addr_t) << std::endl;
		addr_t.s_addr = src_ip;
		std::cout << "src_ip:"<< inet_ntoa(addr_t) << std::endl;
		if(dst_ip == m_vir_ip.s_addr)
		{
 			///Reach the destination
			std::cout << "reach destination" << std::endl;
			if(write(tun_fd, data_recv_buffer, nread-sizeof(DataMessage)) < 0)
			{
				perror ("Writing tun");
				exit(1);
			}
    ...
    {
        ///Forwarding(uint32_t dstIP,uint64_t dstGPS)
    }*/
}
...
IR::build_neighbor(BeaconMessage * message)
{
	...
	if (it != m_neighbor_table.end())
	{
		...
		it->second->gpsq.push(GPSVec::MillierConvertion(gps.x,gps.y));
	}
	else
	{
		//not find, insert a new entry;
		NeighborEntry * entry = new NeighborEntry(gps, until_time, speed, message->m_phy_ip);
		entry->gpsq.push(GPSVec::MillierConvertion(gps.x,gps.y));
