	...
	else
	{
		...
		if ((gps_data.status == STATUS_FIX) && 
		        ...
		        !isnan(gps_data.fix.longitude))
			{
				...
				//need modify
				m_speed.x = gps_data.fix.speed;
				m_speed.y = gps_data.fix.speed;
...
IR::build_beacon(struct BeaconMessage *message)
{
	...
	GPS gps = get_gps();
	message->src_gps_x = GPS::GPSXToUint32(gps);
	message->src_gps_y = GPS::GPSYToUint32(gps);
	message->src_speed_x = m_speed.x;
	message->src_speed_y = m_speed.y;
	gettimeofday(&tv, &tz);
...
IR::recv_tun(int fd)
{
	...
	if(opt)
	{
		...
		int nsend = sendto(data_fd, data_buffer, nread + sizeof(data_message), 0, (struct sockaddr*) &dst_addr, sizeof(dst_addr));//nread + sizeof(message)
